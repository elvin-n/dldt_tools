...
void Int8Calibrator::collectFP32Statistic() {
    ...
    ExecutableNetwork executable_network = _ieI8C.LoadNetwork(network, _targetDeviceI8C, { { CONFIG_KEY(EXCLUSIVE_ASYNC_REQUESTS), CONFIG_VALUE(YES) } });
...
CNNNetwork Int8Calibrator::createICNNNetworkForLayer(CNNLayer::Ptr layerToClone, bool hasReLU) {
    CNNLayer::Ptr layerRelU = layerToClone->outData[0]->getInputTo().begin()->second;

    InferenceEngine::CNNNetReader reader1;
    DataPtr inputData = layerToClone->insData[0].lock();
    std::string inputName = inputData->getName();
    ...
    CNNLayerPtr inputLayer = inputs.begin()->second->getInputData()->getCreatorLayer().lock();
...
void Int8Calibrator::collectByLayerStatistic(const InferenceEngine::NetworkStatsMap &stat) {
    ...
    for (auto l : _layersAccuracyDrop) {
        ...
        if (layerToClone->outData.size() == 1
            && layerToClone->outData[0]->getInputTo().size() == 1
            && CaselessEq<std::string>()(layerToClone->outData[0]->getInputTo().begin()->second->name, "relu")) {
            layerRelU = layerToClone->outData[0]->getInputTo().begin()->second;
        }
        ...
        ExecutableNetwork enetwork = _ieI8C.LoadNetwork(n, _targetDeviceI8C, { { CONFIG_KEY(EXCLUSIVE_ASYNC_REQUESTS), CONFIG_VALUE(YES) } });
        _singleLayerNetworks.push_back(enetwork);
        InferenceEngine::InferRequest request = enetwork.CreateInferRequest();
        std::string inputName = layerToClone->insData[0].lock()->getName();
...
void Int8Calibrator::collectCalibrationStatistic(size_t pics) {
    ...
    if (_collectStatistic) {
        for (auto l : _statData.registeredLayers()) {
            ...
            size_t N, C;
            if (outBlob->getTensorDesc().getDims().size() == 4 && outBlob->getTensorDesc().getLayout() == Layout::NCHW) {
                N = pics;
                C = outBlob->getTensorDesc().getDims()[1];
            } else if (outBlob->getTensorDesc().getDims().size() == 2 && outBlob->getTensorDesc().getLayout() == Layout::NC) {
                N = pics;
                C = outBlob->getTensorDesc().getDims()[1];
            } else {
                continue;
            ...
            for (size_t n = 0; n < N; n++) {
                if (outBlob->getTensorDesc().getDims().size() == 4) {
                    size_t _HW = outBlob->getTensorDesc().getDims()[3] * outBlob->getTensorDesc().getDims()[2];
                ...
                } else if (outBlob->getTensorDesc().getDims().size() == 2) {
                    if (outBlob->getTensorDesc().getPrecision() == Precision::FP32) {
...
ClassificationCalibrator::ClassificationCalibrator(int nPictures, const std::string &flags_m,
                                                   ...
                                                   int flags_b, InferenceEngine::Core ie,
        ...
        for (; b < batch && iter != annCollector.annotations().end(); b++, iter++, filesWatched++) {
            ...
            try {
                ...
                if (scaleProposalToInputSize) {
                    scale_x *= firstInputBlob->getTensorDesc().getDims()[2];
                    scale_y *= firstInputBlob->getTensorDesc().getDims()[3];
                }
