...
static const char calibration_type_message[] = "Select the type of calibration process. 0 - based on network accuracy metric, 1 - KL-Divergence based";
...
DEFINE_int32(calibration_type, 0, calibration_type_message);
/**
...
int main(int argc, char *argv[]) {
    try {
		...
		calibrator->collectFP32ActivationsKL();
		processor->Process(FLAGS_stream_output);
		auto statMap = calibrator->optimizeActivationKL();
		std::string outModelName = FLAGS_output.empty() ? fileNameNoExt(FLAGS_m) + "_i8" : fileNameNoExt(FLAGS_output);
		SaveCalibratedIR(FLAGS_m, outModelName, statMap, FLAGS_convert_fc);
